...
import {
	...
	sidebarDataAccItems,
	sidebarDataApps,
	sidebarDataInterests,
	sidebarAddition,
} from '../../../../helpers/settingsSidebar'
...
const SettingsRightbar = ({ setText }) => {
	...
	useEffect(() => {
		if (activeBtn && activeInput1 && activeInput2 && activeInput3)
			setText('Allow All')
		if (!activeBtn || !activeInput1 || !activeInput2 || !activeInput3)
			setText('Allow some')
		if (!activeBtn && !activeInput1 && !activeInput2 && !activeInput3)
			...
			<div className='settingsRightbar'>
				...
				<div className='settingsRightbarItem'>
					<div className='settingsRightbarItemTop'>
						<span className='settingsRightbarItemTitle'>
							Personalization and data
						</span>
						<button
							className={
								activeBtn
									? 'settingsRightbarItemBtn active'
									: 'settingsRightbarItemBtn'
							}
									...
									<div key={i.id}>
										<div className='settingsRightbarItemTop'>
											...
											<div className='checkboxBlock'>
												<input
													type='checkbox'
													checked={inputNum(i.id, 'activeInput') ? true : false}
													onChange={inputNum(i.id, 'inputChanging')}
												/>
											</div>
										</div>
				...
				<div className={activeForm ? 'form active' : 'form'}>
					<div className='formBlock'>
						...
						<button className='disableBtn' onClick={onclickDisable}>
							Disable
						</button>
						<button
							className='cancelBtn'
			...
			<div className='settingsRightbar'>
					...
					return (
						<Link
							...
							to={`/settings/your_twitter_data/${
								item.onclick !== '' ? item.onclick : false
							}`}
						>
							<div className='settingsItem specialItem'>
								<h3 className='settingsItemTitle'>{item.title}</h3>
				...
				<div className='settingsItem-unhovered'>
					<p className='settingsRightbarItemText'>
						This information applies to your browser or device while you’re
						logged out. It may be different when you’re logged in.
					</p>
					<span className='settingsRightbarItemText'>
						Control how Twitter customizes your experience in
					</span>
					<span className='addition'>
						your personalization and data settings
					</span>
				</div>
			</div>
		)
			...
			<div className='settingsRightbar'>
				...
				{sidebarDataInterests.map(item => {
					return (
						<Link
							key={item.id}
							to={`/settings/your_twitter_data/${item.onclick}`}
						>
							<div className='settingsItem specialItem'>
								<h3 className='settingsItemTitle'>{item.title}</h3>
								<img
	...
	const DataDownload = () => {
		...
		return (
			<div className='settingsRightbar'>
				...
				<div className='settingsItem-unhovered'>
					<p className='settingsRightbarRequest'>
						Download a file with all the information associated with your
	...
	const DataAccGen = () => <SettingsDataAcc props={sidebarDataAccItems[0]} />
	const DataAccAge = () => <SettingsDataAcc props={sidebarDataAccItems[1]} />
		...
		return (
			<div className='settingsRightbar'>
				...
				<div className='settingsItem-unhovered'>
					...
					<p className='settingsCenteredText'>
	...
	const DataInterestsTwitter = () => {
		...
		return (
			<div className='settingsRightbar'>
				...
				<div className='settingsItem-unhovered'>
					<p className='settingsRightbarPostTitle'>
						These are some of the interests matched to you based on your
						profile, activity, and the Topics you follow. These are used to
						personalize your experience across Twitter, including the ads you
						see. You can adjust your interests if something doesn’t look right.
						Any changes you make may take a little while to go into effect.
					</p>
				</div>
				<div className='settingsItem-unhovered'>
					<h1 className='settingsCenteredTitle'>Nothing to see here — yet</h1>
	...
	const DataInterestsPartners = () => {
		...
		return (
			<div className='settingsRightbar'>
				<div className='settingsRightbarTop'>
					<h1 className='settingsRightbarTitle'>
						Inferred interests from partners
					</h1>
				</div>
				<p className='settingsCenteredTitle'>
					There is nothing now. Come back later
				</p>
			</div>
		)
	}
	const DataInterestsAudiences = () => {
		...
		return (
			<div className='settingsRightbar'>
				...
				<div className='settingsItem-unhovered'>
					<span className='settingsRightbarRequest'>
						Tailored audiences are often built from email lists or browsing
						behaviors. They help advertisers reach prospective customers or
				...
				<div className='settingsItem-unhovered'>
					<span className='settingsRightbarRequest'>
						You can opt out of interest-based advertising in your
						personalization and data settings. This will change the ads you see
						on Twitter, however it won’t remove you from advertisers’ audiences.
					</span>
				</div>
			</div>
		)
							...
							{item.items.map(i => {
								return (
									<div key={i.id} className='settingsItem specialItem'>
										...
										<img
											className='settingsItemImg'
											src={PF + 'icon/utility/arrowUpRight.svg'}
											alt=''
										/>
									</div>
								)
	...
	return (
		<>
			...
			{location.pathname === '/settings' ? (
				<Navigate to='/settings/account/personalization' />
			) : null}
			<Routes>
				...
				<Route
					path='/your_twitter_data/request_data'
				...
				<Route
					path='/your_twitter_data/twitter_interests'
					element={<DataInterestsTwitter />}
				/>
				<Route
					path='/your_twitter_data/partner_interests'
					element={<DataInterestsPartners />}
				/>
				<Route
					path='/your_twitter_data/audiences'
					element={<DataInterestsAudiences />}
				/>
